    ...
    function GetPreNext($gtype='')
    {
        ...
        if(count($this->PreNext)<2)
        {
            ...
            if(is_array($preRow))
            {
                ...
                $this->PreNext['pre'] = "<a href='$mlink' class='a'>{$preRow['title']}</a> ";
                $this->PreNext['preimg'] = "<a href='$mlink' class='a'></a> "; 
            }
            else
            {
                $this->PreNext['pre'] = "<a href='#' class='a'>没有了</a> ";
                $this->PreNext['preimg'] ="<a href='javascript:;' class='a'></a>";
            }
            if(is_array($nextRow))
            {
                $mlink = GetFileUrl($nextRow['id'],$nextRow['typeid'],$nextRow['senddate'],$nextRow['title'],$nextRow['ismake'],$nextRow['arcrank'],
                $nextRow['namerule'],$nextRow['typedir'],$nextRow['money'],$nextRow['filename'],$nextRow['moresite'],$nextRow['siteurl'],$nextRow['sitepath']);
                $this->PreNext['next'] = "<a href='$mlink' class='b'>{$nextRow['title']}</a> ";
                $this->PreNext['nextimg'] = "<a href='$mlink' class='b'></a>";
            }
            else
            {
                $this->PreNext['next'] = "<a href='#' class='b'>没有了</a>  ";
                $this->PreNext['nextimg'] ="<a href='javascript:;' class='b'></a>";
            }
        }
        if($gtype=='pre')
        {
